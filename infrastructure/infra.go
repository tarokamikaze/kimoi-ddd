...
func fromEntity(e domain.EntityUser) ActiveRecordUser {
	...
	return ActiveRecordUser{
		...
		// データ整合性を担保するためにはDDDのEntityのproperty を一部 package private にしたい
		// よってDBのデータを直接受けるActive Record(DTO?) 的な Struct (Class) はインフラ層で定義して
		// public propertyな状態にする必要がある

		// 一方でDDD的には、ドメイン層からインフラ層への依存はご法度。
		// つまり、インフラ層でEntityのprivate propertyから無理やり値を取得してActive Recordに反映するか
		// Entityの private property に全部getterを生やすしかない
		// つらい
